...
public class Simulator : MonoBehaviour
{
    ...
    public PayloadFromHost payload = new PayloadFromHost();
    private void Awake()
    {
    }
    ...
    public PayloadFromHost getReadyCmd()
    {
        PayloadFromHost a = new PayloadFromHost();
        a.units = units;
        a.time = time;
        return a;
    }
    ...
    void ProcessMyCommand()
    {
        for (int i = 0; i < commands.Count; i++)
        {
            ...
            if (c is UnitCmd)
            {
                UnitCmd c1 = (UnitCmd)c;
                Unit u = GetUnit(c1.uuid);
                u.vx = c1.vx;
                u.vz = c1.vz;
                Debug.Log("Sim");
            }
