            ...
            for (int j = i; j < targets.Count; j++)
            {
                ...
                if (0 < d && d < 1)
                {
                    ...
                    if (UnitType.isProjectile(u1.type)) // tinaxd u1 is arrow or fireball
                    {
                        ...
                        collision1.myDamage = Mathf.Floor(Mathf.Abs(collision.normalVelocity));
                        u1.vx1 = u1.vx;
                        u1.vz1 = u1.vz;
                        u1.HP = 0;
                    ...
                    {
                        if (u1.owner == u2.owner) continue;
                        collision.myDamage = Mathf.Floor(Mathf.Abs(collision.normalVelocity));
                    ...
                    else // neither u1 nor u2 is arrow or fireball
                    {
                        ...
                        if (u1vel > u2vel)
                        {
                            collision1.myDamage = Mathf.Floor(5 * Mathf.Abs(collision.normalVelocity));
                            collision.myDamage = Mathf.Floor(collision1.myDamage / 2);
                        }
                        else
                        {
                            collision.myDamage = Mathf.Floor(5 * Mathf.Abs(collision.normalVelocity));
                            collision1.myDamage = Mathf.Floor(collision.myDamage / 2);
                        }
